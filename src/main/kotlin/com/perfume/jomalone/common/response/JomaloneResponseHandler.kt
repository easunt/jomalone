...
//TODO :: change
@RestControllerAdvice("com.perfume.jomalone")
class JomaloneResponseHandler : ResponseBodyAdvice<Any> {
    ...
    override fun beforeBodyWrite(body: Any?, returnType: MethodParameter, selectedContentType: MediaType, selectedConverterType: Class<out HttpMessageConverter<*>>, request: ServerHttpRequest, response: ServerHttpResponse): Any {
        return when (body) {
            null -> JomaloneResponse(ApiResult.OK)
            is JomaloneResponse -> body
            else -> JomaloneResponse(ApiResult.OK, body)
        }
    }
}